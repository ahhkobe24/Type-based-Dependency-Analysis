...
///////////////////////////////////////////////////////////////////
// DependAnal -declares new parsing rules and actions            //
// Ver 1.0                                                       //
...
{  
  ...
  public:
    ...
    void doAction(const Scanner::ITokCollection* pTc) override
    {
      ...
      //std::string debug = pTc->show();
      ASTNode* top = p_Repos->scopeStack().top();size_t nameIndex = pTc->find("(") - 1;std::string name = (*pTc)[nameIndex];
      // is function a destructor?
      if (nameIndex > 0 && (*pTc)[nameIndex - 1] == "~")
      {--nameIndex;name = "~" + name;}
      top->type_ = "function";top->name_ = name;top->package_ = p_Repos->package();
      ...
      {//----< start find class name >--------------------
        ...
        * - leave function ASTNode on stack top as it may have child nodes        */
        ...
        ASTNode* pFunctNode = p_Repos->scopeStack().top();ASTNode* pParentNode = p_Repos->scopeStack().predOfTop();
        pParentNode->children_.pop_back();           // unlink function
        pClassNode->children_.push_back(pFunctNode); // relink function
        return;
      }// is this a lambda?
      std::string packageName = p_Repos->package();std::string ext = FileSystem::Path::getExt(packageName);
      size_t posOpenBracket = pTc->find("[");size_t posCloseBracket = pTc->find("]");size_t posBrace = pTc->find("{");
      ...
      {std::string name;
        ...
        top->name_ = name;top->type_ = "lambda";
      }
    }
  };
  ...
  public:
    ...
    void doAction(const Scanner::ITokCollection* pTc) override
    {
      ...
      if (operIndex < pTc->length())
      {
        name = "operator" + (*pTc)[operIndex + 1] + (*pTc)[operIndex + 2];top->name_ = name;nameIndex = operIndex;
      }

      // is lambda?

      size_t posParen = pTc->find("(");size_t posBrace = pTc->find("{");size_t posEqual = pTc->find("=");
      if (posParen < posBrace && posBrace < pTc->length())
      {
        if (posEqual < pTc->length() - 1 && (*pTc)[posEqual + 1] == ">")
        {
          std::string name;
          for (size_t i = posParen; i <= posBrace; ++i)name += (*pTc)[i];
          top->name_ = name;top->type_ = "lambda";
        }
      }
    }
  ...
  */
  ...
  public:
    bool doTest(const Scanner::ITokCollection* pTc) override
    {
      ...
      std::string debug3 = pTc->show();Scanner::SemiExp tc;tc.clone(*pTc);
      // begin added 2/26/2017
      Access access = pRepo->currentAccess();bool isPublic = false;std::string parentType = pRepo->scopeStack().top()->type_;
      if (pTc->find("private") < pTc->length())
      {isPublic = false;pRepo->currentAccess() = Access::priv;}
      if (pTc->find("protected") < pTc->length())
      {isPublic = false;pRepo->currentAccess() = Access::prot;}
      if (pTc->find("public") < pTc->length() && parentType != "function")
      {isPublic = true;pRepo->currentAccess() = Access::publ;}
      // end added 2/26/2017
      if (tc.length() > 0 && tc[0] == "using")
      {doActions(pTc);return IRule::Stop;}
  ...
  public:
    ...
    void doAction(const Scanner::ITokCollection* pTc) override
    {
      ...
      {
		  GrammarHelper::removeFunctionArgs(se);
		  GrammarHelper::removeQualifiers(se);
